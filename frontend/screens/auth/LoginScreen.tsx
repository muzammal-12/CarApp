...
export default function LoginScreen({ navigation }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      setBusy(true);
      await login({ email: email.trim(), password }); // auth logic unchanged
    } catch (e: any) {
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        >
          ...
          >
            ...
            <Text style={styles.pageSub}>Sign in to continue</Text>

            {/* Dark card (synchronized with bg) */}
...
const styles = StyleSheet.create({
  ...
  // dark/glass card to sync with gradient
  card: {
    width: "100%",
    maxWidth: 520,
    backgroundColor: "rgba(255,255,255,0.06)",
